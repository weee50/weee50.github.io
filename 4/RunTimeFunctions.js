    ...
    for(j = 0; j < sizeY; j++){
      if(GameState == 3){
        if(field[i][j].mines != 0){
          ...
          cont.drawImage(Textures.Color[field[i][j].color], screenscale * i, screenscale * j, screenscale, screenscale);
          if (field[i][j].color > 0 && field[i][j].neighbours == 0) {
            cont.drawImage(Textures.Ground[33], screenscale * i, screenscale * j, screenscale, screenscale);
          } else {
...
function digAround(posX, posY){
  ...
  for(i = 0; i < z.length; i++){
    ...
    if(!field[sx][sy].open){
        ...
        if(field[sx][sy].neighbours == 0 && field[sx][sy].color == 0){
            digAround(sx,sy);
        }
    }
  }
  ...
  for(x = 0; x < sizeX; x++){
    for(y = 0; y < sizeY; y++){
        ...
        for(i = 0; i < z.length; i++){
            sx = z[i][0];
            sy = z[i][1];
            if (field[sx][sy].mines == 1) {
              field[x][y].neighbours += 1;
            } else if (field[sx][sy].mines == -1) {
              field[x][y].color += 1;
            } else if (field[sx][sy].mines == 2) {
              field[x][y].neighbours += 1;
              field[x][y].color += 1;
            }
        }
...
function leftClick(posX, posY){
  ...
  if(field[posX][posY].mines != 0){
    ...
    if(field[posX][posY].neighbours == 0 && field[posX][posY].color == 0){
      digAround(posX, posY);
    }
  }
  drawStuff();
...
function rightClick(posX, posY){
  ...
  if (field[posX][posY].flags == 0) {
    field[posX][posY].flags = 1;
  } else if (field[posX][posY].flags == 1) {
    field[posX][posY].flags = -1;
  } else if (field[posX][posY].flags == -1) {
    field[posX][posY].flags = 2;
  } else {
    field[posX][posY].flags = 0;
  }
